...
impl SynthesisEngine {
    ...
    async fn detect_trends(&self, memories: &[Memory]) -> Option<Insight> {
        ...
        let first_span = if first_half.len() >= 2 {
            (first_half.last().unwrap().created_at - first_half.first().unwrap().created_at)
                .num_days()
        } else {
            1
        };
        ...
        let second_span = if second_half.len() >= 2 {
            (second_half.last().unwrap().created_at - second_half.first().unwrap().created_at)
                .num_days()
        } else {
            1
        };
    ...
    pub async fn find_related(
        ...
        for candidate in candidates {
            ...
            let intersection: std::collections::HashSet<_> =
                memory_words.intersection(&candidate_words).collect();
            let union: std::collections::HashSet<_> =
                memory_words.union(&candidate_words).collect();
