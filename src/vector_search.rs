...
impl VectorIndex {
    ...
    pub async fn init(&self) -> Result<()> {
        std::fs::create_dir_all(&self.config.index_path)
            .map_err(|e| MemoryError::Storage(format!("Failed to create vector index dir: {}", e)))?;
        Ok(())
    }
    ...
    pub async fn search(&self, query_embedding: &[f32], limit: usize) -> Result<Vec<(MemoryId, f32)>> {
    ...
    pub async fn delete(&self, memory_id: &MemoryId) -> Result<()> {
        ...
        if index_file.exists() {
            tokio::fs::remove_file(&index_file).await
